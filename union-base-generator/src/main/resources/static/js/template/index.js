$(document).ready(function() {
    layui.use(['form', 'table','layer'], function() {
        ...
        table.render({
            ...
            height: 500,
            url: '/template/manager/list',
            cols: [[
                {field: 'templateName', minWidth: 160,  title: '模版名称'},
                {field: 'templatePath',  title: '模版文件', templet: '#templatePathTpl'},
                {field: 'templateFileSize',  minWidth: 100, title: '模版大小', templet: '#templateFileSizeTpl'},
                {field: 'engineName',  title: '模版引擎', templet: '#engineNameTpl'},
                {field: 'templateType',  title: '模版类型', templet: '#templateTypeTpl'},
        ...
        $("#add_template_btn").click(function() {
           window.location.href = '/template/manager/add';
        ...
        table.on('tool(listDataTable01Filter)', function(obj) {
            var data = obj.data;
            if (obj.event === 'detail') {
                window.location.href = "/template/manager/detail/" + data.id;
            } else if (obj.event === 'edit') {
                window.location.href = "/template/manager/update/" + data.id;
            } else if (obj.event === 'delete') {
                layer.confirm("确定删除？", {icon: 3}, function(index) {
                    $.ajax({
                        url: "/template/manager/delete/" + data.id,
                        type: "DELETE",
                        dataType: "json",
                        success: function(data) {
                            if (data.success) {
